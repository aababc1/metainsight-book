...
In this approach, after pre-processing, where fastq files are processed to enhance sequence quality, alignment is performed by mapping the reads to the reference sequences*. The STAR [](doi:10.1093/bioinformatics/bts635) program is utilized to create the reference file and perform the mapping of reads to the generated reference sequences.  
...
To align the mapping information after read mapping, the samtools [](doi:10.1093/bioinformatics/btp352) `view` and `sort` functions are utilized to transform the .sam files into .bam files. 
...
Once these .bam files are generated, you can visualize the alignment results using the IGV [](doi:10.1038/nbt.1754) program. To examine the alignment results with the IGV program, you will need the MAG.fa obtained from metagenome de novo assembly or MAG results. If you wish to display annotated coding sequences (CDS) information within the MAG.fa sequences, you can import an annotation-generated .gff file* as well.
...
```
Using Bedtools [](doi:10.1093/bioinformatics/btq033), you can obtain coverage (count) information for each CDS (coding sequence) in functional annotations. Before applying Bedtools, it is advisable to simplify the .gff entries to include only the essential information for Bedtools usage.
